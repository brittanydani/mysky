...
describe('getStreakStatus()', () => {
  ...
  it('returns zeros with empty check-ins', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue([]);
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(0);
  ...
  it('detects today check-in → streak of 1', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0]));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(1);
  ...
  it('calculates a 5-day consecutive streak', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0, 1, 2, 3, 4]));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(5);
  ...
  it('breaks streak on a gap (days 0, 1, 3, 4 → streak = 2)', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0, 1, 3, 4]));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(4);
  ...
  it('detects at-risk when yesterday checked in but not today', async () => {
    ...
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([1, 2, 3]));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(3);
  ...
  it('detects 7-day milestone', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0, 1, 2, 3, 4, 5, 6]));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(7);
  ...
  it('detects 30-day milestone', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns(Array.from({ length: 30 }, (_, i) => i)));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(30);
  ...
  it('returns non-milestone null for streak of 8', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns(Array.from({ length: 8 }, (_, i) => i)));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(8);
  ...
  it('returns safe defaults when localDb throws', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockRejectedValue(new Error('db failure'));
  ...
  it('daysSinceLastCheckIn is 2 when last check-in was 2 days ago', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([2, 3, 4]));
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(3);
...
describe('getWeeklyReflection()', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (supabaseDb.getJournalEntries as jest.Mock).mockResolvedValue([]);
  });
  ...
  it('hasEnoughData is false with fewer than 3 check-ins this week', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0, 1]));
    (supabaseDb.getSleepEntries as jest.Mock).mockResolvedValue([]);
  ...
  it('hasEnoughData is true with 3+ check-ins this week', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0, 1, 2]));
    (supabaseDb.getSleepEntries as jest.Mock).mockResolvedValue([]);
  ...
  it('detects moodDirection "up" when this week > last week by > 0.3', async () => {
    ...
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue([...thisWeek, ...lastWeek]);
    (supabaseDb.getSleepEntries as jest.Mock).mockResolvedValue([]);
  ...
  it('calculates avgSleep from this week sleep entries', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(makeCheckIns([0, 1, 2]));
    (supabaseDb.getSleepEntries as jest.Mock).mockResolvedValue(makeSleepEntries([0, 1, 2], 8));
  ...
  it('returns safe defaults when localDb throws', async () => {
    (supabaseDb.getCheckIns as jest.Mock).mockRejectedValue(new Error('db error'));
...
describe('getDailyLoopData()', () => {
  ...
  it('prioritizes milestone insights over other available insight types', async () => {
    ...
    (supabaseDb.getCheckIns as jest.Mock).mockResolvedValue(streakCheckIns);
    (supabaseDb.getTotalCheckInCount as jest.Mock).mockResolvedValue(7);
    (supabaseDb.getSleepEntries as jest.Mock).mockResolvedValue(makeSleepEntries([0, 1, 2, 3, 4, 5, 6], 8));
