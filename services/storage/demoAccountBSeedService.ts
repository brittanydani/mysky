...
import { ACCOUNT_B_DEMO_SEED, ACCOUNT_B_REFLECTIONS } from './demoAccountBSeed';
import { VALUES_QUESTIONS, ARCHETYPE_QUESTIONS, COGNITIVE_QUESTIONS, INTELLIGENCE_QUESTIONS } from '../../constants/dailyReflectionQuestions';
  ...
  async _seedSettingsAndStorage(): Promise<void> {
    ...
    if (validAnswers.length > 0) {
      ...
      await EncryptedAsyncStorage.setItem('@mysky:daily_reflections', JSON.stringify({
        ...
        totalDaysCompleted: Math.max(1, dayNumber(new Date(lastDateStr)) - dayNumber(new Date(firstDateStr)) + 1),
