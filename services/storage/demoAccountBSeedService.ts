...
const SIMPLE_PHASES: ('new' | 'waxing' | 'full' | 'waning')[] = ['waxing','waxing','full','waning','waning','new'];

function uid(): string { return Crypto.randomUUID(); }
function isoDate(d: Date): string { return toLocalDateString(d); }
function dateFromISODateString(dateStr: string): Date { return new Date(`${dateStr}T12:00:00.000Z`); }
function dayNumber(d: Date): number { return Math.floor(d.getTime() / 86400000); }
function simpleMoonPhaseForIndex(i: number): 'new' | 'waxing' | 'full' | 'waning' { return SIMPLE_PHASES[i % SIMPLE_PHASES.length]; }
...
export const DemoAccountBSeedService = {
  ...
  async sendDemoDataToSupabase(email: string | null | undefined): Promise<void> {
    if (!DemoAccountBSeedService.isDemoAccount(email)) throw new Error('Demo data send is only available for the Account B demo user.');
    ...
    const checkInRows = ACCOUNT_B_DEMO_SEED.dailyEntries.flatMap((entry) => [
      { user_id: user.id, mood_value: entry.morningMood, log_date: entry.date, time_of_day: 'morning', created_at: new Date(`${entry.date}T09:00:00.000Z`).toISOString(), updated_at: new Date(`${entry.date}T09:00:00.000Z`).toISOString() },
      { user_id: user.id, mood_value: entry.eveningMood, log_date: entry.date, time_of_day: 'evening', created_at: new Date(`${entry.date}T20:00:00.000Z`).toISOString(), updated_at: new Date(`${entry.date}T20:00:00.000Z`).toISOString() },
    ]);
    ...
    const { error: checkInError } = await supabase.from('daily_check_ins').upsert(checkInRows, { onConflict: 'user_id,log_date,time_of_day' });
    if (checkInError) throw new Error(checkInError.message || 'Failed to upload demo check-ins.');

    const { error: dailyLogError } = await supabase.from('daily_logs').upsert(dailyLogRows, { onConflict: 'user_id,log_date' });
    if (dailyLogError) throw new Error(dailyLogError.message || 'Failed to upload demo daily logs.');
  ...
  async _seedHistoricalEntries(): Promise<void> {
    for (let i = 0; i < ACCOUNT_B_DEMO_SEED.dailyEntries.length; i += 1) {
      ...
      await supabaseDb.saveCheckIn({
        ...
        moonHouse: 1 + ((i + 1) % 12),
        sunHouse: 1 + ((i + 7) % 12),
        transitEvents: [{ transitPlanet: 'Moon', natalPlanet: 'Saturn', aspect: 'opposition', orb: 2.1, isApplying: false }],
  ...
  async _seedSettingsAndStorage(): Promise<void> {
    await supabaseDb.saveSettings({ id: uid(), cloudSyncEnabled: false, createdAt: CHART_CREATED, updatedAt: CHART_CREATED });
    ...
    await EncryptedAsyncStorage.setItem('@mysky:somatic_entries', JSON.stringify(ACCOUNT_B_DEMO_SEED.somaticEntries.map((entry) => ({ id: uid(), date: entry.date, region: entry.region, emotion: entry.emotion, intensity: entry.intensity, note: entry.note, trigger: entry.trigger ?? '', whatHelped: entry.whatHelped ?? '' }))));
    await EncryptedAsyncStorage.setItem('@mysky:trigger_events', JSON.stringify(ACCOUNT_B_DEMO_SEED.triggerEvents.map((entry) => ({ id: uid(), timestamp: new Date(entry.date).getTime(), mode: entry.mode, event: entry.event, nsState: entry.nsState, sensations: entry.sensations, note: entry.note }))));
    await EncryptedAsyncStorage.setItem('@mysky:relationship_patterns', JSON.stringify(ACCOUNT_B_DEMO_SEED.relationshipPatterns.map((entry) => ({ id: uid(), date: entry.date, note: entry.note, tags: entry.tags }))));
    await EncryptedAsyncStorage.setItem('@mysky:relationship_charts', JSON.stringify(ACCOUNT_B_DEMO_SEED.relationshipCharts.map((entry) => ({ id: uid(), name: entry.name, relationship: entry.relationship, birthDate: entry.birthDate, birthTime: entry.birthTime, hasUnknownTime: entry.hasUnknownTime, birthPlace: entry.birthPlace, latitude: entry.latitude, longitude: entry.longitude, timezone: entry.timezone, dynamicNote: entry.dynamicNote }))));
    ...
    if (validAnswers.length > 0) {
      const answersToSave = validAnswers.map((r) => {
        ...
        return { questionId: foundQ ? foundQ.id : 0, category: r.category, questionText: r.questionText, answer: r.answer, scaleValue: SCALES[r.answer] ?? 1, date: r.date, sealedAt: new Date(`${r.date}T21:00:00.000Z`).toISOString() };
      });
      const firstDateStr = `${validAnswers[0].date}T12:00:00.000Z`;
      const lastDateStr = `${validAnswers[validAnswers.length - 1].date}T12:00:00.000Z`;
      await EncryptedAsyncStorage.setItem('@mysky:daily_reflections', JSON.stringify({ answers: answersToSave, totalDaysCompleted: Math.max(1, dayNumber(new Date(lastDateStr)) - dayNumber(new Date(firstDateStr)) + 1), startedAt: new Date(firstDateStr).toISOString() }));
    }
  },
  ...
  async _dailyTopUp(): Promise<void> {
    ...
    await supabaseDb.saveSleepEntry({ id: `demo-topup-sleep-${today}`, chartId, date: today, durationHours: todayRow.sleepHours, quality: DemoAccountBSeedService._sleepQualityFromHours(todayRow.sleepHours), dreamText: '', dreamFeelings: '[]', dreamMetadata: '{}', createdAt: new Date(`${today}T08:00:00.000Z`).toISOString(), updatedAt: new Date(`${today}T08:00:00.000Z`).toISOString(), isDeleted: false });
    await supabaseDb.saveCheckIn({ id: `demo-checkin-${today}-morning`, date: today, chartId, timeOfDay: 'morning', moodScore: todayRow.morningMood, energyLevel: todayRow.morningEnergy, stressLevel: todayRow.morningStress, tags: todayRow.morningTags, note: todayRow.morningNote, wins: todayRow.morningWin, challenges: todayRow.morningChallenge, moonSign: MOON_SIGNS[0], moonHouse: 1, sunHouse: 7, transitEvents: [], lunarPhase: LUNAR_PHASES[0], retrogrades: [], createdAt: new Date(`${today}T09:00:00.000Z`).toISOString(), updatedAt: new Date(`${today}T09:00:00.000Z`).toISOString() });
    await supabaseDb.saveCheckIn({ id: `demo-checkin-${today}-evening`, date: today, chartId, timeOfDay: 'evening', moodScore: todayRow.eveningMood, energyLevel: todayRow.eveningEnergy, stressLevel: todayRow.eveningStress, tags: todayRow.eveningTags, note: todayRow.eveningNote, wins: todayRow.eveningWin, challenges: todayRow.eveningChallenge, moonSign: MOON_SIGNS[2], moonHouse: 2, sunHouse: 8, transitEvents: [], lunarPhase: LUNAR_PHASES[1], retrogrades: [], createdAt: new Date(`${today}T19:00:00.000Z`).toISOString(), updatedAt: new Date(`${today}T19:00:00.000Z`).toISOString() });
  ...
  async _repairUnreadableDemoSeedData(): Promise<boolean> {
    ...
    const { data: rows, error } = await supabase.from('journal_entries').select('id, title_enc, content_enc').eq('user_id', userId).like('id', 'demo-journal-%');
    if (error) { logger.warn('[DemoSeed] Unable to scan demo journals for repair:', error); return false; }

    const typedRows = (rows ?? []) as Array<{ id: string; title_enc: string | null; content_enc: string | null }>;
    const unreadableIds: string[] = [];
    for (const row of typedRows) {
      const titleResult = row.title_enc ? await FieldEncryptionService.tryDecryptField(row.title_enc) : { ok: true as const, value: '' };
      const contentResult = row.content_enc ? await FieldEncryptionService.tryDecryptField(row.content_enc) : { ok: true as const, value: '' };
      if (!titleResult.ok || !contentResult.ok) unreadableIds.push(row.id);
    }
    if (unreadableIds.length === 0) return false;

    logger.warn(`[DemoSeed] Found ${unreadableIds.length} unreadable demo journal entr${unreadableIds.length === 1 ? 'y' : 'ies'}; reseeding Account B.`, unreadableIds.slice(0, 5));
    await DemoAccountBSeedService._seed();
