...
export function warnBirthProfileSyncUnavailable(error: unknown) {
  ...
  logger.warn(
    '[SyncService] Birth profile remote sync is unavailable or misconfigured; using local cache fallback where possible.',
    error,
  );
}

async function invokeBirthProfileSyncViaEdge(
...
export async function invokeBirthProfileSync(
  ...
  try {
    return await invokeBirthProfileSyncDirect(action, payload);
  } catch (directTableError) {
    if (!isBirthProfileSyncUnavailableError(directTableError)) {
      throw directTableError;
    }

    try {
      return await invokeBirthProfileSyncViaEdge(action, payload);
    } catch (edgeFunctionError) {
      if (action === 'getLatest' && swallowUnavailableReadError) {
...
async function invokeBirthProfileSyncDirect(
  ...
  if (action === 'getLatest') {
    ...
    if (!profile || (since && profile.updatedAt <= since)) return { profile: null };
    return { profile };
  }
  ...
  if (action === 'upsert') {
    ...
    if (error) throw error;
    return { profile: { ...profile, createdAt, isDeleted: false, deletedAt: undefined } };
  }
