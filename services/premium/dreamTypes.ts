/**
 * Dream Interpretation Types — Psychological Engine
 *
 * Core types, constants, and feeling definitions for the evidence-based
 * dream interpretation engine. All interpretation is grounded in:
 *   - Emotional tone (primary signal)
 *   - Nervous system activation patterns
 *   - Attachment dynamics
 *   - Shadow trigger themes
 *   - Recurring emotional patterns over time
 *
 * TONE: Warm but not poetic. Grounded. Never predictive. Never absolute.
 * LANGUAGE: "may", "could", "often reflects" — always non-deterministic.
 */

// ─── Nervous System ───────────────────────────────────────────────────────────

export type NervousSystemBranch =
  | 'ventral_safety'  // integration, connection, healing
  | 'fight'           // boundary activation, power struggle, anger
  | 'flight'          // anxiety, avoidance, fear of loss
  | 'freeze'          // voice suppression, overwhelm, shame
  | 'collapse'        // hopelessness, depletion, grief
  | 'mixed';          // inner conflict or dual states

// ─── Attachment ───────────────────────────────────────────────────────────────

export type AttachmentStyle = 'secure' | 'anxious' | 'avoidant' | 'disorganized';

// ─── Shadow Triggers ──────────────────────────────────────────────────────────

export type ShadowTrigger =
  | 'abandonment'
  | 'rejection'
  | 'betrayal'
  | 'shame'
  | 'exposure'
  | 'control'
  | 'power'
  | 'helplessness'
  | 'danger'
  | 'intimacy'
  | 'sexuality'
  | 'consent_violation'
  | 'worthiness'
  | 'responsibility'
  | 'failure'
  | 'grief'
  | 'identity'
  | 'belonging'
  | 'unpredictability'
  | 'punishment'
  | 'isolation'
  | 'transformation';

// ─── Dream Feelings ───────────────────────────────────────────────────────────

export interface DreamFeelingDef {
  id: string;
  label: string;
  /** Primary nervous system branch this feeling activates */
  primaryBranch: NervousSystemBranch;
  /** Shadow triggers this feeling is most associated with */
  shadowTriggers: ShadowTrigger[];
  /** Attachment style this feeling most strongly signals */
  attachmentSignal: AttachmentStyle;
  /** Positive (+1), negative (−1), or ambivalent (0) valence */
  valence: -1 | 0 | 1;
  /** Activation level: high (1), medium (0.5), low (0) */
  activation: 0 | 0.5 | 1;
  /** UI tier for the tiered feeling selector */
  tier: FeelingTier;
}

/** Tier groupings shown in the feeling selector dropdown */
export type FeelingTier =
  | 'negative'   // "Mostly Negative"
  | 'positive'   // "Mostly Positive"
  | 'mixed'      // "Mixed"
  | 'hard'       // "Hard to name"
  | 'all';       // "All" — show every feeling alphabetically

/**
 * Production-ready 60-feeling set for dream work.
 *
 * Grouped by activation × valence × nervous system state.
 * Covers all 22 shadow triggers, all attachment styles, and all NS branches.
 * Designed for a tiered selector: Mostly Negative → Mostly Positive → Mixed → Hard to name.
 */
export const DREAM_FEELINGS: DreamFeelingDef[] = [
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔴 HIGH ACTIVATION – NEGATIVE  (Fight / Flight)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'anxious',
    label: 'Anxious',
    primaryBranch: 'flight',
    shadowTriggers: ['unpredictability', 'danger'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'panicked',
    label: 'Panicked',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'terrified',
    label: 'Terrified',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'scared',
    label: 'Scared',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'alarmed',
    label: 'Alarmed',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'overwhelmed',
    label: 'Overwhelmed',
    primaryBranch: 'freeze',
    shadowTriggers: ['helplessness', 'responsibility', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'urgent',
    label: 'Urgent',
    primaryBranch: 'flight',
    shadowTriggers: ['responsibility', 'failure', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'stressed',
    label: 'Stressed',
    primaryBranch: 'flight',
    shadowTriggers: ['responsibility', 'control', 'failure'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'agitated',
    label: 'Agitated',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'power', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'frustrated',
    label: 'Frustrated',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'power', 'failure'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'angry',
    label: 'Angry',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'power', 'betrayal'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'enraged',
    label: 'Enraged',
    primaryBranch: 'fight',
    shadowTriggers: ['power', 'betrayal', 'consent_violation'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'defensive',
    label: 'Defensive',
    primaryBranch: 'fight',
    shadowTriggers: ['exposure', 'shame', 'control'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'betrayed',
    label: 'Betrayed',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'abandonment', 'intimacy'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'betrayal',
    label: 'Betrayal',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'intimacy', 'punishment'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'exposed',
    label: 'Exposed',
    primaryBranch: 'freeze',
    shadowTriggers: ['exposure', 'shame', 'worthiness'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'pressured',
    label: 'Pressured',
    primaryBranch: 'flight',
    shadowTriggers: ['responsibility', 'control', 'power'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟠 HIGH ACTIVATION – MIXED / AMBIVALENT
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'yearning',
    label: 'Yearning',
    primaryBranch: 'mixed',
    shadowTriggers: ['abandonment', 'belonging', 'intimacy'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'desperate',
    label: 'Desperate',
    primaryBranch: 'mixed',
    shadowTriggers: ['helplessness', 'abandonment', 'isolation'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'jealous',
    label: 'Jealous',
    primaryBranch: 'mixed',
    shadowTriggers: ['worthiness', 'rejection', 'belonging'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'obsessed',
    label: 'Obsessed',
    primaryBranch: 'mixed',
    shadowTriggers: ['control', 'intimacy', 'identity'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'tempted',
    label: 'Tempted',
    primaryBranch: 'mixed',
    shadowTriggers: ['sexuality', 'consent_violation', 'intimacy'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'electrified',
    label: 'Electrified',
    primaryBranch: 'mixed',
    shadowTriggers: ['sexuality', 'power', 'transformation'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'drawn',
    label: 'Drawn',
    primaryBranch: 'mixed',
    shadowTriggers: ['intimacy', 'belonging', 'sexuality'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'compelled',
    label: 'Compelled',
    primaryBranch: 'mixed',
    shadowTriggers: ['control', 'identity', 'transformation'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'restless',
    label: 'Restless',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'unpredictability', 'transformation'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'conflicted',
    label: 'Conflicted',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'belonging', 'responsibility'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟡 HIGH ACTIVATION – POSITIVE
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'excited',
    label: 'Excited',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'empowered',
    label: 'Empowered',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['power'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'brave',
    label: 'Brave',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['danger', 'power'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'determined',
    label: 'Determined',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'failure'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'passionate',
    label: 'Passionate',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['sexuality', 'intimacy', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'alive',
    label: 'Alive',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'inspired',
    label: 'Inspired',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔵 MEDIUM ACTIVATION – NEGATIVE  (Freeze Emerging)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'uncertain',
    label: 'Uncertain',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'confused',
    label: 'Confused',
    primaryBranch: 'freeze',
    shadowTriggers: ['identity', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'insecure',
    label: 'Insecure',
    primaryBranch: 'freeze',
    shadowTriggers: ['worthiness', 'rejection', 'exposure'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'embarrassed',
    label: 'Embarrassed',
    primaryBranch: 'freeze',
    shadowTriggers: ['exposure', 'shame'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'ashamed',
    label: 'Ashamed',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'exposure', 'rejection'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'guilty',
    label: 'Guilty',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'punishment', 'responsibility'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'rejected',
    label: 'Rejected',
    primaryBranch: 'freeze',
    shadowTriggers: ['rejection', 'worthiness', 'belonging'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'lonely',
    label: 'Lonely',
    primaryBranch: 'freeze',
    shadowTriggers: ['isolation', 'belonging', 'abandonment'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'small',
    label: 'Small',
    primaryBranch: 'freeze',
    shadowTriggers: ['worthiness', 'power', 'helplessness'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'watched',
    label: 'Watched',
    primaryBranch: 'freeze',
    shadowTriggers: ['exposure', 'danger', 'control'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟣 MEDIUM ACTIVATION – POSITIVE
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'hopeful',
    label: 'Hopeful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'curious',
    label: 'Curious',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'connected',
    label: 'Connected',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'intimacy'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'seen',
    label: 'Seen',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'worthiness', 'exposure'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'chosen',
    label: 'Chosen',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'worthiness', 'rejection'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'understood',
    label: 'Understood',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'identity', 'isolation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'supported',
    label: 'Supported',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'helplessness'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟢 LOW ACTIVATION – NEGATIVE  (Freeze / Collapse)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'numb',
    label: 'Numb',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'identity', 'isolation'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'heavy',
    label: 'Heavy',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'responsibility', 'helplessness'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'powerless',
    label: 'Powerless',
    primaryBranch: 'collapse',
    shadowTriggers: ['helplessness', 'power', 'control'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'trapped',
    label: 'Trapped',
    primaryBranch: 'collapse',
    shadowTriggers: ['control', 'helplessness', 'consent_violation'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'hopeless',
    label: 'Hopeless',
    primaryBranch: 'collapse',
    shadowTriggers: ['helplessness', 'grief', 'failure'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'sad',
    label: 'Sad',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'abandonment'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'disappointed',
    label: 'Disappointed',
    primaryBranch: 'collapse',
    shadowTriggers: ['failure', 'rejection', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'isolated',
    label: 'Isolated',
    primaryBranch: 'collapse',
    shadowTriggers: ['isolation', 'belonging', 'abandonment'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'invisible',
    label: 'Invisible',
    primaryBranch: 'collapse',
    shadowTriggers: ['worthiness', 'belonging', 'identity'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟢 LOW ACTIVATION – POSITIVE  (Ventral Safety)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'calm',
    label: 'Calm',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'safe',
    label: 'Safe',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'relieved',
    label: 'Relieved',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['danger', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'grounded',
    label: 'Grounded',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'peaceful',
    label: 'Peaceful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'accepted',
    label: 'Accepted',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'worthiness', 'rejection'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'warm',
    label: 'Warm',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['intimacy', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔥 SEXUAL / SOMATIC CHARGE
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'aroused',
    label: 'Aroused',
    primaryBranch: 'mixed',
    shadowTriggers: ['sexuality', 'intimacy'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'desired',
    label: 'Desired',
    primaryBranch: 'mixed',
    shadowTriggers: ['worthiness', 'sexuality', 'belonging'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'seduced',
    label: 'Seduced',
    primaryBranch: 'mixed',
    shadowTriggers: ['consent_violation', 'power', 'sexuality'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'lustful',
    label: 'Lustful',
    primaryBranch: 'mixed',
    shadowTriggers: ['sexuality', 'control', 'intimacy'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'tender',
    label: 'Tender',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['intimacy', 'belonging', 'grief'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔄 RESTORED FROM PREVIOUS SET
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'abandoned',
    label: 'Abandoned',
    primaryBranch: 'flight',
    shadowTriggers: ['abandonment', 'rejection', 'isolation'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'grieving',
    label: 'Grieving',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'abandonment', 'isolation'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'nostalgic',
    label: 'Nostalgic',
    primaryBranch: 'mixed',
    shadowTriggers: ['grief', 'belonging', 'transformation'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },
  {
    id: 'disgusted',
    label: 'Disgusted',
    primaryBranch: 'fight',
    shadowTriggers: ['consent_violation', 'shame', 'exposure'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'longing',
    label: 'Longing',
    primaryBranch: 'mixed',
    shadowTriggers: ['abandonment', 'belonging', 'intimacy'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // ✨ NEW GAP FILLERS
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'suspicious',
    label: 'Suspicious',
    primaryBranch: 'flight',
    shadowTriggers: ['betrayal', 'danger', 'control'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'protective',
    label: 'Protective',
    primaryBranch: 'fight',
    shadowTriggers: ['responsibility', 'power', 'danger'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'resentful',
    label: 'Resentful',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'punishment', 'control'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'dissociated',
    label: 'Dissociated',
    primaryBranch: 'collapse',
    shadowTriggers: ['identity', 'isolation', 'consent_violation'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'suffocated',
    label: 'Suffocated',
    primaryBranch: 'freeze',
    shadowTriggers: ['control', 'consent_violation', 'helplessness'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'hollow',
    label: 'Hollow',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'identity', 'isolation'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'grateful',
    label: 'Grateful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'worthiness'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'free',
    label: 'Free',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'control'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'helpless',
    label: 'Helpless',
    primaryBranch: 'freeze',
    shadowTriggers: ['helplessness', 'danger', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // ✨ BALANCED / POSITIVE GAP FILLERS
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'playful',
    label: 'Playful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'thoughtful',
    label: 'Thoughtful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'responsibility'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'neutral',
    label: 'Neutral',
    primaryBranch: 'mixed',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔴 ADDITIONAL NEGATIVE – HIGH ACTIVATION
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'fear',
    label: 'Fear',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'violated',
    label: 'Violated',
    primaryBranch: 'fight',
    shadowTriggers: ['consent_violation', 'betrayal', 'exposure'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'humiliated',
    label: 'Humiliated',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'exposure', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'shattered',
    label: 'Shattered',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'betrayal', 'identity'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟠 ADDITIONAL NEGATIVE – MEDIUM ACTIVATION
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'dread',
    label: 'Dread',
    primaryBranch: 'freeze',
    shadowTriggers: ['danger', 'helplessness', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'worthless',
    label: 'Worthless',
    primaryBranch: 'freeze',
    shadowTriggers: ['worthiness', 'rejection', 'shame'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'misunderstood',
    label: 'Misunderstood',
    primaryBranch: 'freeze',
    shadowTriggers: ['identity', 'belonging', 'isolation'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'unheard',
    label: 'Unheard',
    primaryBranch: 'freeze',
    shadowTriggers: ['isolation', 'power', 'control'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'invalidated',
    label: 'Invalidated',
    primaryBranch: 'freeze',
    shadowTriggers: ['isolation', 'worthiness', 'power'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'unprotected',
    label: 'Unprotected',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'abandonment', 'helplessness'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'blamed',
    label: 'Blamed',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'punishment', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'bitter',
    label: 'Bitter',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'punishment', 'rejection'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'wary',
    label: 'Wary',
    primaryBranch: 'freeze',
    shadowTriggers: ['betrayal', 'danger', 'unpredictability'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔵 ADDITIONAL NEGATIVE – LOW ACTIVATION  (Collapse)
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'heartbroken',
    label: 'Heartbroken',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'abandonment', 'intimacy'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'defeated',
    label: 'Defeated',
    primaryBranch: 'collapse',
    shadowTriggers: ['failure', 'helplessness', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🌒 ADDITIONAL MIXED / AMBIVALENT
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'bittersweet',
    label: 'Bittersweet',
    primaryBranch: 'mixed',
    shadowTriggers: ['grief', 'transformation', 'belonging'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },
  {
    id: 'melancholic',
    label: 'Melancholic',
    primaryBranch: 'mixed',
    shadowTriggers: ['grief', 'isolation', 'transformation'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },
  {
    id: 'resigned',
    label: 'Resigned',
    primaryBranch: 'mixed',
    shadowTriggers: ['helplessness', 'control', 'failure'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },
  {
    id: 'eerie',
    label: 'Eerie',
    primaryBranch: 'freeze',
    shadowTriggers: ['danger', 'unpredictability', 'identity'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 0.5,
    tier: 'hard',
  },
  {
    id: 'surreal',
    label: 'Surreal',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'transformation', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 0.5,
    tier: 'hard',
  },
  {
    id: 'raw',
    label: 'Raw',
    primaryBranch: 'mixed',
    shadowTriggers: ['exposure', 'grief', 'transformation'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 0.5,
    tier: 'hard',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🌟 ADDITIONAL POSITIVE – HIGH ACTIVATION
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'joyful',
    label: 'Joyful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'triumphant',
    label: 'Triumphant',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['power', 'failure', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'awestruck',
    label: 'Awestruck',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🌟 ADDITIONAL POSITIVE – MEDIUM ACTIVATION
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'proud',
    label: 'Proud',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['worthiness', 'power'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🌟 ADDITIONAL POSITIVE – LOW ACTIVATION
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'loved',
    label: 'Loved',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'intimacy', 'worthiness'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'content',
    label: 'Content',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'cherished',
    label: 'Cherished',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['worthiness', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'nurtured',
    label: 'Nurtured',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'helplessness'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },

  // ── Fear / Threat ─────────────────────────────────────────────────────────
  {
    id: 'afraid',
    label: 'Afraid',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'nervous',
    label: 'Nervous',
    primaryBranch: 'flight',
    shadowTriggers: ['unpredictability', 'danger'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'uneasy',
    label: 'Uneasy',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'danger'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'frightened',
    label: 'Frightened',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'paranoid',
    label: 'Paranoid',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'danger', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'startled',
    label: 'Startled',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'unsafe',
    label: 'Unsafe',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'threatened',
    label: 'Threatened',
    primaryBranch: 'fight',
    shadowTriggers: ['danger', 'control', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'vulnerable',
    label: 'Vulnerable',
    primaryBranch: 'freeze',
    shadowTriggers: ['exposure', 'helplessness', 'shame'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ── Stress / Overwhelm ────────────────────────────────────────────────────
  {
    id: 'burdened',
    label: 'Burdened',
    primaryBranch: 'collapse',
    shadowTriggers: ['responsibility', 'helplessness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'tense',
    label: 'Tense',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'strained',
    label: 'Strained',
    primaryBranch: 'freeze',
    shadowTriggers: ['responsibility', 'helplessness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'exhausted',
    label: 'Exhausted',
    primaryBranch: 'collapse',
    shadowTriggers: ['responsibility', 'helplessness'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'drained',
    label: 'Drained',
    primaryBranch: 'collapse',
    shadowTriggers: ['helplessness', 'responsibility'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'frazzled',
    label: 'Frazzled',
    primaryBranch: 'flight',
    shadowTriggers: ['control', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'rushed',
    label: 'Rushed',
    primaryBranch: 'flight',
    shadowTriggers: ['control', 'responsibility'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'unsettled',
    label: 'Unsettled',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'danger'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'irritated',
    label: 'Irritated',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'edgy',
    label: 'Edgy',
    primaryBranch: 'fight',
    shadowTriggers: ['danger', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ── Sadness / Grief ───────────────────────────────────────────────────────
  {
    id: 'devastated',
    label: 'Devastated',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'abandonment', 'rejection'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'empty',
    label: 'Empty',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'isolation', 'belonging'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'miserable',
    label: 'Miserable',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'helplessness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'down',
    label: 'Down',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'worthiness'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'depressed',
    label: 'Depressed',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'isolation', 'helplessness'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'hurt',
    label: 'Hurt',
    primaryBranch: 'freeze',
    shadowTriggers: ['rejection', 'betrayal', 'grief'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ── Anger / Frustration ───────────────────────────────────────────────────
  {
    id: 'furious',
    label: 'Furious',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'betrayal', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'annoyed',
    label: 'Annoyed',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'offended',
    label: 'Offended',
    primaryBranch: 'fight',
    shadowTriggers: ['shame', 'rejection', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'indignant',
    label: 'Indignant',
    primaryBranch: 'fight',
    shadowTriggers: ['power', 'shame', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'hostile',
    label: 'Hostile',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'danger', 'control'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'aggressive',
    label: 'Aggressive',
    primaryBranch: 'fight',
    shadowTriggers: ['power', 'control', 'danger'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'impatient',
    label: 'Impatient',
    primaryBranch: 'flight',
    shadowTriggers: ['control', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'provoked',
    label: 'Provoked',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'power', 'betrayal'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },

  // ── Shame / Self-judgment ─────────────────────────────────────────────────
  {
    id: 'regretful',
    label: 'Regretful',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'responsibility', 'failure'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'self_critical',
    label: 'Self-critical',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'worthiness', 'failure'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'inadequate',
    label: 'Inadequate',
    primaryBranch: 'freeze',
    shadowTriggers: ['worthiness', 'failure', 'shame'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },

  // ── Confusion / Disorientation ────────────────────────────────────────────
  {
    id: 'puzzled',
    label: 'Puzzled',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'disoriented',
    label: 'Disoriented',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'lost',
    label: 'Lost',
    primaryBranch: 'freeze',
    shadowTriggers: ['identity', 'belonging', 'helplessness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'bewildered',
    label: 'Bewildered',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'questioning',
    label: 'Questioning',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'doubtful',
    label: 'Doubtful',
    primaryBranch: 'freeze',
    shadowTriggers: ['worthiness', 'failure', 'identity'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'unclear',
    label: 'Unclear',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },

  // ── Surprise / Shock ──────────────────────────────────────────────────────
  {
    id: 'surprised',
    label: 'Surprised',
    primaryBranch: 'mixed',
    shadowTriggers: ['unpredictability'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'shocked',
    label: 'Shocked',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'danger'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'stunned',
    label: 'Stunned',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability', 'danger'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'amazed',
    label: 'Amazed',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'speechless',
    label: 'Speechless',
    primaryBranch: 'freeze',
    shadowTriggers: ['unpredictability'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
  {
    id: 'intrigued',
    label: 'Intrigued',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'wondering',
    label: 'Wondering',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },

  // ── Joy / Positive emotions ───────────────────────────────────────────────
  {
    id: 'happy',
    label: 'Happy',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'delighted',
    label: 'Delighted',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'thrilled',
    label: 'Thrilled',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'cheerful',
    label: 'Cheerful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'blissful',
    label: 'Blissful',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'euphoric',
    label: 'Euphoric',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'lighthearted',
    label: 'Lighthearted',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'satisfied',
    label: 'Satisfied',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },

  // ── Love / Connection ─────────────────────────────────────────────────────
  {
    id: 'loving',
    label: 'Loving',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['intimacy', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'affectionate',
    label: 'Affectionate',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['intimacy', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'compassionate',
    label: 'Compassionate',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'empathetic',
    label: 'Empathetic',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'intimacy'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'close',
    label: 'Close',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['intimacy', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'trusting',
    label: 'Trusting',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['betrayal', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },

  // ── Safety / Comfort ──────────────────────────────────────────────────────
  {
    id: 'relaxed',
    label: 'Relaxed',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'secure',
    label: 'Secure',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['abandonment', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'comforted',
    label: 'Comforted',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'reassured',
    label: 'Reassured',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['abandonment', 'unpredictability'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'stable',
    label: 'Stable',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['unpredictability'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'protected',
    label: 'Protected',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['danger', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'settled',
    label: 'Settled',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['unpredictability'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },

  // ── Power / Confidence ────────────────────────────────────────────────────
  {
    id: 'confident',
    label: 'Confident',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['worthiness', 'failure'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'courageous',
    label: 'Courageous',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['danger', 'power'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'strong',
    label: 'Strong',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['power', 'helplessness'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'capable',
    label: 'Capable',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['worthiness', 'failure'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'motivated',
    label: 'Motivated',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'focused',
    label: 'Focused',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },

  // ── Curiosity / Exploration ───────────────────────────────────────────────
  {
    id: 'adventurous',
    label: 'Adventurous',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['danger'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'interested',
    label: 'Interested',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'fascinated',
    label: 'Fascinated',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'engaged',
    label: 'Engaged',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'investigative',
    label: 'Investigative',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'reflective',
    label: 'Reflective',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'imaginative',
    label: 'Imaginative',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },

  // ── Wonder / Mystical ─────────────────────────────────────────────────────
  {
    id: 'mystified',
    label: 'Mystified',
    primaryBranch: 'mixed',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'enchanted',
    label: 'Enchanted',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'awed',
    label: 'Awed',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'dreamlike',
    label: 'Dreamlike',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'magical',
    label: 'Magical',
    primaryBranch: 'ventral_safety',
    shadowTriggers: [],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },

  // ── Survival / Threat responses ───────────────────────────────────────────
  {
    id: 'alert',
    label: 'Alert',
    primaryBranch: 'fight',
    shadowTriggers: ['danger', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'hypervigilant',
    label: 'Hypervigilant',
    primaryBranch: 'fight',
    shadowTriggers: ['danger', 'betrayal', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'escaping',
    label: 'Escaping',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness', 'control'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'hiding',
    label: 'Hiding',
    primaryBranch: 'freeze',
    shadowTriggers: ['danger', 'shame', 'exposure'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'guarded',
    label: 'Guarded',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'danger', 'intimacy'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'on_edge',
    label: 'On edge',
    primaryBranch: 'fight',
    shadowTriggers: ['danger', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'cornered',
    label: 'Cornered',
    primaryBranch: 'fight',
    shadowTriggers: ['danger', 'helplessness', 'control'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },

  // ── Loss / Separation ─────────────────────────────────────────────────────
  {
    id: 'missing_someone',
    label: 'Missing someone',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'abandonment', 'belonging'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'mourning',
    label: 'Mourning',
    primaryBranch: 'collapse',
    shadowTriggers: ['grief', 'abandonment'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'homesick',
    label: 'Homesick',
    primaryBranch: 'collapse',
    shadowTriggers: ['belonging', 'grief', 'abandonment'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'forgotten',
    label: 'Forgotten',
    primaryBranch: 'collapse',
    shadowTriggers: ['abandonment', 'belonging', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'let_down',
    label: 'Let down',
    primaryBranch: 'collapse',
    shadowTriggers: ['betrayal', 'rejection', 'grief'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'unwanted',
    label: 'Unwanted',
    primaryBranch: 'freeze',
    shadowTriggers: ['rejection', 'abandonment', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'replaced',
    label: 'Replaced',
    primaryBranch: 'freeze',
    shadowTriggers: ['rejection', 'abandonment', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'distant',
    label: 'Distant',
    primaryBranch: 'collapse',
    shadowTriggers: ['isolation', 'belonging'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },

  // ── Transformation / Growth ───────────────────────────────────────────────
  {
    id: 'changed',
    label: 'Changed',
    primaryBranch: 'mixed',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'evolving',
    label: 'Evolving',
    primaryBranch: 'mixed',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'renewed',
    label: 'Renewed',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'reborn',
    label: 'Reborn',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'transformed',
    label: 'Transformed',
    primaryBranch: 'mixed',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'healing',
    label: 'Healing',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'grief'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'awakening',
    label: 'Awakening',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'liberated',
    label: 'Liberated',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['control', 'transformation', 'power'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'releasing',
    label: 'Releasing',
    primaryBranch: 'mixed',
    shadowTriggers: ['transformation', 'control'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },

  // ── Surreal / Dream states ────────────────────────────────────────────────
  {
    id: 'floating',
    label: 'Floating',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'detached',
    label: 'Detached',
    primaryBranch: 'collapse',
    shadowTriggers: ['isolation', 'identity'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'unreal',
    label: 'Unreal',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'unpredictability'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 0.5,
    tier: 'hard',
  },
  {
    id: 'otherworldly',
    label: 'Otherworldly',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'lucid',
    label: 'Lucid',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0.5,
    tier: 'hard',
  },
  {
    id: 'spaced_out',
    label: 'Spaced out',
    primaryBranch: 'collapse',
    shadowTriggers: ['identity', 'isolation'],
    attachmentSignal: 'avoidant',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'abstract',
    label: 'Abstract',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'timeless',
    label: 'Timeless',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'hard',
  },

  // ── Mixed / Ambiguous ─────────────────────────────────────────────────────
  {
    id: 'torn',
    label: 'Torn',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'belonging', 'betrayal'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },
  {
    id: 'ambivalent',
    label: 'Ambivalent',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },
  {
    id: 'mixed_emotions',
    label: 'Mixed emotions',
    primaryBranch: 'mixed',
    shadowTriggers: ['identity', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },
  {
    id: 'uncomfortable',
    label: 'Uncomfortable',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'exposure', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'strange',
    label: 'Strange',
    primaryBranch: 'mixed',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'anxious',
    valence: 0,
    activation: 0.5,
    tier: 'mixed',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔴 RELATIONAL NEGATIVE — Things done TO you in dreams
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'silenced',
    label: 'Silenced',
    primaryBranch: 'freeze',
    shadowTriggers: ['control', 'power', 'isolation'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'paralyzed',
    label: 'Paralyzed',
    primaryBranch: 'freeze',
    shadowTriggers: ['helplessness', 'danger', 'control'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'deceived',
    label: 'Deceived',
    primaryBranch: 'fight',
    shadowTriggers: ['betrayal', 'intimacy', 'control'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'neglected',
    label: 'Neglected',
    primaryBranch: 'collapse',
    shadowTriggers: ['abandonment', 'belonging', 'worthiness'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'punished',
    label: 'Punished',
    primaryBranch: 'freeze',
    shadowTriggers: ['punishment', 'shame', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'mocked',
    label: 'Mocked',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'exposure', 'rejection'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'controlled',
    label: 'Controlled',
    primaryBranch: 'freeze',
    shadowTriggers: ['control', 'consent_violation', 'helplessness'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'discarded',
    label: 'Discarded',
    primaryBranch: 'collapse',
    shadowTriggers: ['worthiness', 'rejection', 'abandonment'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },
  {
    id: 'degraded',
    label: 'Degraded',
    primaryBranch: 'freeze',
    shadowTriggers: ['shame', 'consent_violation', 'power'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'coerced',
    label: 'Coerced',
    primaryBranch: 'freeze',
    shadowTriggers: ['consent_violation', 'control', 'helplessness'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'erased',
    label: 'Erased',
    primaryBranch: 'collapse',
    shadowTriggers: ['identity', 'worthiness', 'isolation'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'haunted',
    label: 'Haunted',
    primaryBranch: 'freeze',
    shadowTriggers: ['grief', 'danger', 'punishment'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'hard',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🟢 RELATIONAL POSITIVE — Things felt WITH or FROM others in dreams
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'held',
    label: 'Held',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'intimacy'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'forgiven',
    label: 'Forgiven',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['shame', 'punishment', 'worthiness'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'welcomed',
    label: 'Welcomed',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'rejection'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'whole',
    label: 'Whole',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'worthy',
    label: 'Worthy',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['worthiness', 'rejection', 'shame'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'embraced',
    label: 'Embraced',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'intimacy', 'isolation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'resilient',
    label: 'Resilient',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['failure', 'helplessness', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'radiant',
    label: 'Radiant',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['identity', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'expansive',
    label: 'Expansive',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['transformation', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'belonging',
    label: 'Belonging',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['belonging', 'isolation', 'rejection'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'anchored',
    label: 'Anchored',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['unpredictability', 'identity'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0,
    tier: 'positive',
  },
  {
    id: 'fearless',
    label: 'Fearless',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['danger', 'power', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🌀 DREAM-SPECIFIC EXPERIENCES
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'chased',
    label: 'Chased',
    primaryBranch: 'flight',
    shadowTriggers: ['danger', 'helplessness', 'unpredictability'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'unprepared',
    label: 'Unprepared',
    primaryBranch: 'freeze',
    shadowTriggers: ['failure', 'exposure', 'shame'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 1,
    tier: 'negative',
  },
  {
    id: 'sinking',
    label: 'Sinking',
    primaryBranch: 'collapse',
    shadowTriggers: ['helplessness', 'grief', 'danger'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'fading',
    label: 'Fading',
    primaryBranch: 'collapse',
    shadowTriggers: ['identity', 'isolation', 'grief'],
    attachmentSignal: 'avoidant',
    valence: -1,
    activation: 0,
    tier: 'hard',
  },
  {
    id: 'fragmented',
    label: 'Fragmented',
    primaryBranch: 'collapse',
    shadowTriggers: ['identity', 'isolation', 'unpredictability'],
    attachmentSignal: 'disorganized',
    valence: -1,
    activation: 0.5,
    tier: 'hard',
  },
  {
    id: 'surrendered',
    label: 'Surrendered',
    primaryBranch: 'mixed',
    shadowTriggers: ['control', 'transformation', 'helplessness'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 0,
    tier: 'mixed',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔗 RELATIONAL — Negative
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'excluded',
    label: 'Excluded',
    primaryBranch: 'freeze',
    shadowTriggers: ['belonging', 'rejection', 'isolation'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0.5,
    tier: 'negative',
  },
  {
    id: 'ignored',
    label: 'Ignored',
    primaryBranch: 'collapse',
    shadowTriggers: ['worthiness', 'isolation', 'belonging'],
    attachmentSignal: 'anxious',
    valence: -1,
    activation: 0,
    tier: 'negative',
  },

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // 🔗 RELATIONAL — Positive
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  {
    id: 'reunited',
    label: 'Reunited',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['grief', 'abandonment', 'belonging'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 1,
    tier: 'positive',
  },
  {
    id: 'guided',
    label: 'Guided',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['helplessness', 'belonging', 'transformation'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'adored',
    label: 'Adored',
    primaryBranch: 'ventral_safety',
    shadowTriggers: ['worthiness', 'belonging', 'intimacy'],
    attachmentSignal: 'secure',
    valence: 1,
    activation: 0.5,
    tier: 'positive',
  },
  {
    id: 'defiant',
    label: 'Defiant',
    primaryBranch: 'fight',
    shadowTriggers: ['control', 'power', 'identity'],
    attachmentSignal: 'secure',
    valence: 0,
    activation: 1,
    tier: 'mixed',
  },
];

/** Quick lookup map: feeling id → DreamFeelingDef */
export const FEELING_MAP: Record<string, DreamFeelingDef> = {};
for (const f of DREAM_FEELINGS) {
  FEELING_MAP[f.id] = f;
}

// ─── Dream Metadata ───────────────────────────────────────────────────────────

export type AwakenState =
  | 'calm'
  | 'anxious'
  | 'scared'
  | 'relieved'
  | 'confused'
  | 'curious'
  | 'sad'
  | 'happy'
  | 'peaceful'
  | 'tired'
  | 'energized'
  | 'shaken'
  | 'disturbed'
  | 'thoughtful'
  | 'inspired'
  | 'numb'
  | 'grateful'
  | 'overwhelmed'
  | 'hopeful'
  | 'unsettled';

/** Overall theme tag for a dream */
export type DreamTheme =
  | 'adventure'
  | 'conflict'
  | 'connection'
  | 'transformation'
  | 'mystery'
  | 'survival'
  | 'loss'
  | 'discovery'
  | 'mundane'
  | 'surreal';

export interface DreamMetadata {
  vividness: number;        // 1–5
  lucidity: number;         // 1–5
  controlLevel: number;     // 1–5 — sense of control / empowerment
  overallTheme?: DreamTheme; // optional overall theme tag
  awakenState: AwakenState;
  recurring: boolean;
}

// ─── User-Selected Feelings ───────────────────────────────────────────────────

export interface SelectedFeeling {
  id: string;          // matches DreamFeelingDef.id
  intensity: number;   // 0–5
}

// ─── Precomputed Aggregates ───────────────────────────────────────────────────

export interface DreamAggregates {
  /** Overall emotional tone: −1 (negative) to +1 (positive) */
  valenceScore: number;
  /** Overall arousal: 'low' | 'moderate' | 'high' */
  activationScore: 'low' | 'moderate' | 'high';
  /** Distribution across attachment styles (sums to ~1.0) */
  attachmentProfile: Record<AttachmentStyle, number>;
  /** Distribution across nervous system branches (sums to ~1.0) */
  nervousSystemProfile: Record<NervousSystemBranch, number>;
  /** Ranked shadow triggers by weight, descending */
  shadowTriggerHeatmap: { trigger: ShadowTrigger; weight: number }[];
  /** Top 2–3 dominant feelings from this dream */
  dominantFeelings: SelectedFeeling[];
  /** Dominant nervous system branch */
  dominantBranch: NervousSystemBranch;
  /** Dominant attachment style */
  dominantAttachment: AttachmentStyle;
}

// ─── Pattern Data ─────────────────────────────────────────────────────────────

export interface DreamPatternData {
  /** Feelings that recur across recent dreams */
  recurringFeelings: string[];
  /** Whether the current dream's dominant feelings are shifting vs prior */
  emotionalTrendDirection: 'increasing' | 'decreasing' | 'stable';
  /** Feeling pairs that frequently co-occur */
  coOccurringPairs: [string, string][];
  /** Number of recent dreams to compare against */
  comparisonCount: number;
}

// ─── Output: Full Interpretation ──────────────────────────────────────────────

export interface DreamInterpretation {
  /** One cohesive paragraph weaving feelings, dream text, triggers, personality, and patterns */
  paragraph: string;
  /** One gentle, open-ended reflection question */
  question: string;
  /** ISO timestamp */
  generatedAt: string;
  /**
   * Explicit imagery labels extracted from the dream text (Phase A).
   * Only includes symbols whose keywords literally appear in the text.
   * Never includes inferred or symbolically expanded labels.
   */
  explicitImagery: string[];
  extractedSymbols?: { word: string; category: string; description: string }[];
  /**
   * Interpretive themes derived from trigger analysis (Phase B).
   * Clearly labeled as interpretation, not factual imagery claims.
   */
  interpretiveThemes: string[];
  /**
   * Dream pattern analysis (when pattern engine produces a confident result).
   * Provides the primary dream pattern, secondary patterns, emotional
   * contradictions, ending analysis, and an elegant undercurrent label.
   */
  patternAnalysis?: {
    primaryPattern: string;
    secondaryPatterns: string[];
    confidence: number;
    undercurrentLabel: string;
    emotionalContradictions: { poleA: string; poleB: string; intensity: number }[];
    endingType: string;
  };
}

// ─── Engine Input ─────────────────────────────────────────────────────────────

export interface DreamInterpretationInput {
  entry: import('../storage/models').SleepEntry;
  dreamText: string;
  feelings: SelectedFeeling[];
  metadata: DreamMetadata;
  aggregates: DreamAggregates;
  patterns: DreamPatternData;
  /** Optional suffix appended to the deterministic seed so RE-INTERPRET can produce a fresh variant. */
  seedSuffix?: string;
}
