 ...
 *
 * Transforms raw logs into dayKey-based mood, stress, energy, journal, and
 * sleep aggregates. It does not choose insights or user-facing copy.
 */

import { DailyCheckIn, EnergyLevel, StressLevel } from '../patterns/types';
import { JournalEntry, SleepEntry } from '../storage/models';
import { DailyAggregate, DailyAggregationInput, DailyAggregationResult } from './types';
...
/**
 * Normalize raw logs, aggregate by day, and derive a chart profile if supplied.
 *
 * Pure function. Caller passes raw data and gets aggregate rows for charting,
 * correlations, exports, and context around the active insight router.
 */
export function buildDailyAggregation(input: DailyAggregationInput): DailyAggregationResult {
...
// ─────────────────────────────────────────────────────────────────────────────
// Re-export normalization helpers for focused aggregation tests.
// ─────────────────────────────────────────────────────────────────────────────
