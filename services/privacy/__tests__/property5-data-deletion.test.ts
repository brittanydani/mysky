// Property-based test for complete data deletion
// **Feature: astrology-app-critical-fixes, Property 5: Complete data deletion**
...
jest.mock('expo-crypto', () => ({
  ...
  getRandomBytesAsync: jest.fn(async (len: number) => new Uint8Array(len).fill(4)),
  randomUUID: jest.fn(() => {
...
describe('Property 5: Complete data deletion', () => {
  ...
  test('**Feature: astrology-app-critical-fixes, Property 5: Complete data deletion**', () => {
    return fc.assert(fc.asyncProperty(
      ...
      async (charts, journals, settings) => {
        ...
        const result = await handler.handleDataDeletionRequest();

        expect(result.success).toBe(true);

        const exportedAfterDelete = await handler.handleDataExportRequest();
        expect(exportedAfterDelete.package?.charts.length).toBe(0);
        expect(exportedAfterDelete.package?.journalEntries.length).toBe(0);
        expect(exportedAfterDelete.package?.settings).toBeNull();
      }
    ), {
      numRuns: 50,
      verbose: true,
