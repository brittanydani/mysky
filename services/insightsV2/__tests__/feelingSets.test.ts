...
describe('insightsV2 feeling sets', () => {
  it('keeps the provided feeling-set copy as the canonical list', () => {
    ...
    expect(FEELING_SET_BY_KEY.anxiety.sentences).toEqual([
      `Worry can make everything feel closer than it is, as if something needs your attention right now.`,
      `Your mind may try to solve worry by thinking ahead, even when there isn’t a clear problem yet.`,
      `Worry often shows up as preparation — scanning, planning, anticipating — even when nothing has gone wrong.`,
      `It can be hard to tell the difference between possibility and probability when worry is active.`,
      `Your body may feel the urgency before your mind understands why.`,
      `Worry can make you feel responsible for outcomes you don’t fully control.`,
      `Trying to eliminate worry completely can sometimes make it louder.`,
      `Worry may not mean something is wrong — it may mean something feels uncertain.`,
      `It can help to bring your attention back to what is actually happening, not just what could happen.`,
      `Worry may ease when your body feels safe, not just when your thoughts feel resolved.`,
    ]);
    ...
    expect(FEELING_SET_BY_KEY.restlessness.sentences).toEqual([
      ...
      `Sometimes restlessness is worry. Sometimes it is energy that needs a direction.`,
