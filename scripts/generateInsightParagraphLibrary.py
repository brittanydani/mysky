...
def paragraph_allowed_surfaces_for_category(category: str, target_surfaces: list[str]) -> list[str]:
    domain_key, _subcategory = CATEGORY_DOMAIN_MAP.get(category, ("humanisticNeeds", "unmetNeedRecognition"))
    allowed = set(taxonomy_allowed_surfaces(DOMAIN_BY_KEY[domain_key]))
    return [surface for surface in target_surfaces if surface in allowed]
...
def with_full_metadata(item: dict[str, object]) -> dict[str, object]:
    ...
    next_item.setdefault(
        "allowedSurfaces",
        paragraph_allowed_surfaces_for_category(str(next_item.get("category", "")), ["today", "patterns"]),
    )
    return next_item
            ...
            items.append(
                {
                    ...
                    **domain_metadata_for_category(bank.category),
                    ...
                    "signalTypes": bank.signal_types,
                    "tags": bank.tags,
                    "allowedSurfaces": paragraph_allowed_surfaces_for_category(bank.category, ["weeklyDeepDive", "thisWeek"]),
                    "avoidIfRecentlyUsed": True,
                    "isCurated": False,
                    "source": "pythonGenerated",
                    "body": body,
